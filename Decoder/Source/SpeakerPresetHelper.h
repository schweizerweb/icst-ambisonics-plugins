...
class SpeakerPresetHelper : public PresetHelper
{
public:
    SpeakerPresetHelper(File _presetDirectory, ActionListener* _pActionListener, ScalingInfo* _pScaling) : PresetHelper(_presetDirectory, _pActionListener), pScalingInfo(_pScaling)
    {
    }

    String UniqueActionMessageSelectPreset() override { return "SelectSpeakerPreset"; }
    String UniqueActionMessageSavePreset() override { return "SaveSpeakerPreset"; }
    ...
    void restoreDefaultsInternal() override
    {
        ...
        createPreset("ICST DOM", {
            ...
            { 0.0, 0.0, 1.0, "29", false, 3 }
        });
        
		notifyPresetListChanged();
    }
    
    void loadDefaultPreset(AmbiSpeakerSet* pSpeakerSet)
    {
        for(File f : presetFiles)
        {
            if(f.getFileNameWithoutExtension() == DEFAULT_PRESET_NAME)
            {
                loadFromXmlFile(f, pSpeakerSet);
                break;
            }
        }
    }
    
    bool loadFromXmlFile(const File file, AmbiSpeakerSet* pSpeakerSet)
    {
    ...
    bool writeToXmlFile(const File file, AmbiSpeakerSet* pSpeakerSet)
    {
    ...
    void createPreset(String name, std::initializer_list<SSpeaker> speakerList)
    {
        ...
        writeToXmlFile(file, speakerSet.get());
